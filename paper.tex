...
\newcommand{\thetitle}{On the Inadequacy of Uniform Modeling for WLAN Workload Generation}
...
\newcommand{\class}[1]{\textsf{\footnotesize{#1}}}
\newcommand{\submodel}[1]{\textsf{\footnotesize{#1}}}
\newcommand{\model}[1]{#1}
...
\end{center}
\vspace{-0.5em}
\caption{Classes and their attributes in the Traffic Object Model.}
\vspace{-2em}
\end{table}
...
In this section, we present a generic object-oriented framework for implementing traffic generation models. This framework is called the Traffic Object Model (\caps{TOM}). It defines four levels of responsibility: network, node, flow and packet. The framework is designed so that different behaviors at each level can be naturally implemented  as classes in object-oriented programming languages. The framework automatically gains the advantages of object orientation, in particular modularity and encapsulation. Every traffic model is naturally split into separate spheres of responsibility, which constitute sub-models for the aspects of the model's behavior at each layer. Thanks to encapsulation and polymorphism of objects, the sub-models from different models can immediately be recombined to create new hybrid behaviors. It is trivial, for example, to produce trace-based node behavior, coupled with parametric flow behavior, and uniform packet behavior. Not all combinations of sub-models are valid, however; which classes may be combined depends on the details of each class. The ability to mix-and-match aspects of behavior at different levels greatly enhances our ability to explore the space of possible traffic models.

%For example, to use uniform flow behavior but non-uniform, trace-based node behavior, simply mix the the uniform implementation of the 

Abstract base classes define the properties at each level that traffic models must implement.\footnote{In Java or C\#, this role may be played either by an actual abstract class or by an interface. In duck-typed languages such as Ruby or Python, no actual abstract class need exist, since type is implicitly defined by usage; if inheritance is desired, the base class can be made abstract by defining stub methods that throw informative exceptions if called.} Figure~\ref{fig:properties} enumerates all the properties exposed by traffic models used in this work. Other traffic models may necessitate exposing additional properties, but this list suffices to implement the models used in this paper. Concrete classes implement the behavior of specific traffic models at each level. A complete synthetic workload is created as follows:
\begin{enumerate}
\item instantiate a \class{Network} object of the appropriate class;
\item the top-level \class{Network} object in turn instantiates a collection of \class{Node} objects and associated \class{Flow} objects;
