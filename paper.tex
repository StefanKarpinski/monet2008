...
Three types of simplifying assumptions are commonly applied for both analysis and generation:
\begin{enumerate}
...
\item \textbf{Independence:} that traffic properties share a single marginal\marginalnote distribution across the entire network, and individual values are independently drawn from this marginal distribution. For example, assuming that a single distribution describes an entire network, as opposed to modeling each flow with its own distribution of packet sizes.
...
Prior research has indicated that, given accurate per-flow distributions, this is a realistic generative model of network behavior~\cite{Karpinski07:realism}.\realismnote
...
Other aspects of a flow's behavior can be represented vectorially as well, but there is a one subtlety that may not be immediately obvious.

By definition, vectors can be composed via addition, yielding a new vector representing the aggregate of the two constituent vectors.
If we are to represent flow behaviors as vectors, then we must ensure that our representation respects addition of vectors: the sum of two flow representations should represent the aggregated behavior of the two flows.
We demonstrate how this can be accomplished and argue that the natural generalization of this approach is to represent an entire network's behavior as a large matrix, with each row representing a single flow.
We call such a representation of flow behavior a \emph{linear representation} since it possesses the fundamental characteristic of linearity:
\begin{align}
  f(x + y) = f(x) + f(y).
\end{align}

The matrix representation gives an uncollapsed representation of network behavior that preserves the non-uniformity and non-independence of flow behaviors.
