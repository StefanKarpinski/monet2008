...
\begin{enumerate}
\item \textbf{Uniform:} that traffic properties are uniformly distributed. For example, assuming that  all nodes in a network have equal probability of being the source or destination of each flows.
...
Hern\'andez-Campos's dissertation~\cite{Hernandez06:dissertation} addresses in great depth how to model the feedback between the network and applications for most \caps{TCP} applications.
Hern\'andez-Campos shows how to abstract an application's behavior from a \caps{TCP} trace and replay it in a manner that accurately reflects how most common applications would react in response to different network conditions.
This research has subsequently been turned into a \caps{TCP} flow generation tool called \tmix~\cite{Weigle06}.
It is important to note that {\tmix} does not provide models for application behaviors or for mixtures of applications in networks.
...
Our work complements this perfectly: this paper provides the means to analyze traffic and generate realistic whole-network application workloads which can then serve as the necessary input to \tmix.
...
Source and destination nodes are treated marginally: an empirically derived marginal frequencies of sources and destination nodes (independently) are used to choose endpoints for each flow.
...
\caps{UDP} flows are modeled as being constant bit-rate (\latin{i.e.} having uniform packet behavior), and \caps{TCP} flows are all treated as file transfers, making them effectively uniform with bandwidth limited by simulated \caps{TCP} dynamics.
...
While Harpoon may be sufficiently realistic for the intended purpose of generating Internet backbone traffic, it is difficult to know this:
generated traffic is only compared with trace traffic using metrics that are explicitly specified in the models.
Realism is never double-checked using derived statistical metrics or actual performance metrics.
For the purpose of generating local-area traffic for wireless experiments, it is certainly not sufficiently realistic. Previous research~\cite{Karpinski07:cbr-failure} has shown that \caps{CBR} packet behavior models, like those used in Harpoon, drastically distort vital performance metrics at all levels of the protocol stack in wireless network simulations. This paper gives equal cause for concern about marginal traffic models.

% There is no criterion proposed to determine whether these properties characterize the original traffic adequately---we can only hope that this approximation is good enough. For many purposes, it likely is sufficient; in particular, it is probably appropriate for the intended use in generating traffic for Internet backbone simulations. Wireless networks, however, are particularly sensitive to workload conditions, and sampling from a limited set of empirical distributions does not suffice to reproduce realistic network-wide traffic.
