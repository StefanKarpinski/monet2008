...
\item \textbf{Independence:} that traffic properties share a single marginal distribution across the entire network, and individual values are independently drawn from this marginal distribution. For example, assuming that a single distribution describes an entire network, as opposed to modeling each flow with its own distribution of packet sizes.
\item \textbf{Subdivided Independence:} that traffic properties behave independently, but only within certain groupings. The assumption, for example, that all flows emanating from each single node share the same distribution of packet sizes.
\end{enumerate}
We would clearly be setting up a straw man if we implied that researchers who use these assumptions actually believe them to be true. They clearly are not: flows do not all share a single packet size distribution; nodes are not uniformly likely sources. These assumptions are applied because the drastically simplify both analysis and workload generation. The question remains, however, whether these assumptions are benign. Is the analysis and generation of network traffic drastically distorted by such assumptions? This is the first fundamental question that this paper addresses.
