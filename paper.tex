...
We believe that two major breakthroughs are needed to bring a much needed revolution to the world of traffic modeling. The first requirement is an objective standard of realism by which to compare traffic models. Such a standard has already been introduced in~\cite{Karpinski07:realism} and used to demonstrate that uniform traffic modeling, including the ubiquitous \caps{CBR} model, drastically distort performance metrics at all levels of the network protocol stack~\cite{Karpinski07:cbr-failure}. The second required breakthrough is a unified framework for expressing traffic models that allows researchers to systematically express and understand the assumptions and transformations entailed in each model.

In this paper, we present a highly general model for representing detailed full-network traffic patterns algebraically. The General Matrix Model (\caps{GMM}) represents the behavior or each network flow as a vector in a high-dimensional linear space, and represents total network behavior as a large matrix whose rows are the behavior vectors. The crucial property of this representation is that it respects vector space operations: summation of behavior vectors gives an accurate description of the aggregate behavior of the flows represented by those vectors. The benefit of this representation is that it makes it immediately possible to express a broad variety of assumptions and conditions on traffic patterns in terms of simple matrix operations. It also allows provides great mathematical insight and allows us to bring all the tools of modern linear algebra to bear on the problem of traffic modeling.
...
{\includegraphics[width=7in]{plots/Application/delay.pdf}}
\subfloat[\textbf{Application:} Average Received Throughput]%
{\includegraphics[width=7in]{plots/Application/throughput.pdf}}
\subfloat[\textbf{Application:} Total Received Data]%
{\includegraphics[width=7in]{plots/Application/total_recv_packets.pdf}}
\subfloat[\textbf{Network:} AODV Control Overhead]%
{\includegraphics[width=7in]{plots/Network/control_packets.pdf}}
\subfloat[\textbf{Link:} 802.11 Control Overhead]%
{\includegraphics[width=7in]{plots/Link/overhead.pdf}}%
