...
\newcommand{\thetitle}{Linear Representation of Wireless Workload}
\newcommand{\theauthors}{%
...
\usepackage{cite,amsmath,amssymb,rotating,multirow,bigstrut,url,wrapfig,relsize}
...
One of the major challenges of modeling full-network traffic is the manifest multilevel interdependence of behaviors.
...
\footnote{The term ``marginal'' refers to the margins of actuarial tables formerly used for statistical computations. The rows and columns of the table are disjoint classes of two metrics. Each entry in a table contains a count of the number of events falling into the joint category for that row and column. The margins contain sums of the rows or columns, thus giving the distribution of classes along each dimension \emph{independent} of each other.}
...
\end{enumerate}
We would clearly be arguing against a straw man if we implied that researchers who apply these assumptions actually believe them to be true.
They clearly are not: flows do not all share a single packet size distribution; nodes are not uniformly likely sources.
These assumptions are applied because they drastically simplify both analysis and workload generation.
There is no accepted, proven technique to represent or analyze the statistical interdependence of traffic patterns other than assuming that these interdependencies do not exist. Worse still, such assumptions are often made implicitly and never stated outright.
