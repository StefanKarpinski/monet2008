...
The assumption, for example, that all flows emanating from a common source node share the same packet size distribution is a conditional model, conditioned on source nodes.
\end{enumerate}
...
Modeling such distributions of distributions realistically for entire networks is currently an unsolved problem; we provide the means to address it.

Fundamentally, uniform, marginal and conditional models are applied to reduce the dimensionality of network behaviors drastically, making analysis and generation tractable problems.
From this perspective, two questions immediately present themselves:
\begin{enumerate}
\item Can we represent ``unreduced'' network behavior in such a manner that analysis and generation are still tractable problems?
...
A linear representation of traffic is a vector-valued function on flows such that the following linearity condition is satisfied:
\begin{quote}
...
To address the question of whether there are better ways to reduce the dimensionality of network behaviors, we first make a simple observation.
In linear representations of traffic, uniform, marginal and conditional assumptions are assertions about rows and columns of the representation matrix being identical. 
Moreover, real traffic matrices can be transformed into uniform, marginal or conditional forms through  very simple matrix multiplications.
...
We argue that matrix factorization, which is nondestructive, is a far superior to multiplication as a way to reduce the dimensionality of network traffic behaviors.
To demonstrate this approach, we use non-negative matrix factorization (\caps{NMF}) to find a low-dimensional approximation of the packet size and inter-packet interval behavior matrices.
This factorization extracts a small set of ``basic behaviors'' from a traffic matrix and simultaneously explains each flow's observed behavior as a mixture of these basic behaviors.
Although the basic behaviors are found without examining port numbers or protocols, they nevertheless correspond to intuitive network activities such as file transfer, typing via \caps{SSH}, web surfing, and ping traffic.
...
The interaction of network users and application behavior with the lower layers of the networking stack is characterized by where, when, how much, and to whom data is transmitted. The joint pattern of traffic generation and mobility through time and space completely determines the effect of wireless usage on the lower levels of the network. This is due to the data-agnostic nature of the protocol stack: by design, \caps{IP} networks treat all data in the same manner~\cite{Clark88}.\QoSnote The credibility of conclusions derived from simulation or experimental deployment depends crucially on our confidence that the models used to generate traffic in experiments are sufficiently realistic. \Figure{trace-vs-uniform} demonstrates how drastically different network traffic appears under uniformity assumptions at the node, flow and packet level of behavior. Marginality assumptions are not as visually dramatic, but we will demonstrate that they distort the performance characteristics of networks almost as much, and sometimes more, than uniform models.

While this work is not inherently specific to wireless networks, since the modeling techniques developed here equally applicable to traffic in wired networks, the results are of fundamental importance to wireless research. Realistic traffic models have a drastic impact on the experimental results in wireless networks~\cite{Karpinski07:realism,Karpinski07:cbr-failure}. This is primarily because wireless networks are locally resource constrained. Compare this with the typical situation in wired \caps{LAN}s, where gigabit Ethernet and modern routers and switches can easily handle all but the most intense levels of traffic. If a wired network operator expects larger volumes of traffic on a regular basis, the solution is to upgrade the hardware, not to change the network protocols. In wireless networks, on the other hand, the physical medium cannot be upgraded: protocols must be improved to make better usage of the medium. Being able to perform experiments with realistic workload is a vital part of the ongoing process of improving wireless protocols. Without realistic workload models for experiments, performance comparisons may be misleading or even completely wrong~\cite{Karpinski07:cbr-failure}.% Better traffic models are necessary to avoid stumbling in the dark while attempting to improve wireless technologies.

Paxson and Floyd observed~\cite{Paxson95} that the interaction between endpoint behavior and the network conditions is inherently \textit{closed-loop} in the sense that the exhibited behavior is governed by a closed feedback loop.
% Traffic models typically attempt to preserve the closed-loop behavior of network traffic.
In the case of most non-\caps{TCP} applications, this feedback does not occur: an open-loop model suffices.
Hern\'andez-Campos' dissertation~\cite{Hernandez06:dissertation} addresses in great depth how to model the feedback between the network and applications for typical \caps{TCP} applications.
Hern\'andez-Campos shows how to abstract an application's behavior from a \caps{TCP} trace and replay it in a manner that accurately reflects how typical applications react in response to different network conditions.
...
Our research complements this perfectly: we provide the means to analyze traffic and generate realistic whole-network application mixtures which can then serve as the necessary input to {\tmix}.

% This presents a fundamental difficulty, however, in that it presumes that we know the intent of endpoints: what \textit{would} they have done under different conditions? While we can speculate about what an individual node might hypothetically do, we currently hardly understand the impact of the full-network traffic pattern upon performance metrics at all---even without trying to account for hypothetical reactions to alternate situations. A fuller understanding of total network behavior must be reached before we can sensibly tackle the complexities of application and user feedback.

% Although we do not attempt to model application and user feedback, we do simulate the effect of the network layer and below on the dynamics of \caps{TCP} for flows that were originally transmitted using \caps{TCP}. Hern\'andez-Campos~\cite{Hernandez06:dissertation} addresses in great depth how to abstract an application's behavior from a \caps{TCP} trace and replay it in a manner that accurately reflects how many common applications would react in response to network conditions. This research has subsequently been turned into a \caps{TCP} flow generation tool for \caps{NS-2}, called \tmix~\cite{Weigle06}. This work is complementary to ours: {\tmix} does not provide a model for application behaviors; it only provides the ability to reproduce accurate closed-loop dynamics given an accurate application behavior model. Our work our work provides the means to analyze traffic and generate realistic application workloads which can then serve as the required inputs to \tmix.

% This paper attempts to provide a first-order approximation of complete network behavior by studying the response of performance metrics to \textit{open-loop} traffic models without feedback from the network to applications and users. It is important to realize that while this does not provide a final picture, we currently lack even a first-order understanding of the effect of different workloads on performance. This first-order understanding is an essential initial step.
...
The properties reproduced are the empirical distributions of the following: ``file size, inter-connection time, source and destination IP ranges, number of active sessions.''
Harpoon exemplifies the uniform and marginal modeling approaches.
Source and destination nodes are treated marginally: empirically derived marginal frequencies of sources and destination nodes (independently) are used to choose endpoints for each flow.
File sizes (\latin{i.e.} total flow sizes) are likewise treated marginally: each flow's total byte  count is sampled from a marginal distribution of flow sizes.
\caps{UDP} flows are modeled as being constant bit-rate (\latin{i.e.} having uniform packet behavior), and \caps{TCP} flows are all treated as file transfers, making them effectively uniformly behaved with bit-rate determined by \caps{TCP} dynamics.
Inter-connection times (\latin{i.e.} per node-pair inter-flow intervals) are sampled from a single empirically derived marginal distribution.
While Harpoon may be sufficiently realistic for the intended purpose of generating Internet backbone traffic,\harpoonnote for the purpose of generating local-area traffic for wireless experiments, it is not.
Previous research~\cite{Karpinski07:cbr-failure} has shown that \caps{CBR} packet behavior models used in Harpoon drastically distort vital performance metrics at all levels of the protocol stack in wireless network simulations.
This paper shows that marginal traffic models also severely misrepresent performance metrics.

Like Harpoon, \caps{D-ITG}~\cite{Avallone04} exemplifies the common uniform and marginal modeling assumptions.
Packet behavior is modeled using a handful of standard statistical distributions (``constant, uniform, normal, cauchy, pareto, exponential, etc.'') for packet size and inter-packet interval distributions, from which packets are generated using repeated independent sampling.
Guidance is not provided, however, for choosing among these distributions, choosing parameter values for individual flows, or selecting a mixture of distributions for flows across an entire network.
Moreover, the mechanism for choosing end-points and start and end times for flows is not described; presumably it is explicitly specified by the user. 
The focus of this project is primarily on the engineering challenge of generating very large volumes of synthetic Internet-like backbone traffic and injecting this traffic into real networks in a distributed fashion.
While \caps{D-ITG} provides the mechanism for generating large volumes of network traffic, it does not provide any real guidance as to what traffic to generate.

\FHC~\cite{Hernandez06:wlan-traffic} have defined the state of the art in parametric modeling of wireless traffic patterns.
They use rigorous statistical methods to fit parametric distributions to a number of properties of wireless traffic instances:
% They propose the following models:
flow arrival processes fit a non-stationary Poisson arrival model; flow inter-arrival intervals (per session) fit a Lognormal distribution;
flows per session fit a BiPareto model; flow sizes fit a BiPareto model.
However, all the proposed models are marginal:
a single statistical model applies to all flows and nodes (packet behavior is not considered).
% One intuitively obvious heterogeneous nature of network traffic:
Each node has a different number of flows assigned to it, but otherwise nodes are identical. 
The flows associated with each node are statistically identical:
flow sizes are drawn from a single marginal distribution, and nothing else distinguishes them.
%, unconditional of their associated node.
% This is a classic example of the limits of marginal modeling.
Yet, intuitively it is clear that different nodes in a network have different tendencies for flow size, flow initiation, and packet behavior.
...
treating all flows and nodes statistically identically severely misrepresents important wireless performance metrics in experiments.

% The focus of this project, however, is on providing the infrastructure to generate very large volumes of synthetic Internet-like backbone traffic. No analysis is provided for determining the realism of traffic mixes, or for choosing flow endpoints realistically. In wireless networks, these factors are of crucial importance to performance, and cannot be overlooked. Both Harpoon and \caps{D-ITG} provide excellent traffic generation platforms, but do not provide a systematic framework for understanding or reproducing realistic whole-network workload in the wireless setting.
