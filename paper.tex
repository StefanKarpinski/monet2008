...
\begin{align}
&\forall\: i > \pkts:   \quad \size_i = 0,      \eqn{zero-packets}\\
&\forall\: i < \pkts:   \quad \ival_i < \infty, \eqn{finite-intervals}\\
&\forall\: i \ge \pkts: \quad \ival_i = \infty. \eqn{infinite-intervals}
\end{align}
% The send time of the $k$th packet can be calculated as
% \begin{align}
% \start + \sum_{i=1}^k \ival_i.
% \end{align}
...
The representation is $\pbd: \Traffic \to \R^3$, defined as
$\pbd(\flow) = \tuple{\pkts,\mathsym{bytes},\mathsym{dur}}$, where
\begin{align}
\mathsym{bytes} &= \sum_{i=1}^{\mathclap{\pkts}}{\size_i} &
\mathsym{dur} &= \sum_{i=1}^{\mathclap{\pkts-1}}{\ival_i}.
\end{align}
This representation encapsulates the information needed to reproduce a \caps{CBR} version of each flow:
each packet should have a $\floor{\mathsym{bytes}/\pkts}$ byte payload, and intervals between packets should be $\mathsym{dur}/\pkts$ seconds.
