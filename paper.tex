...
%\section{Algebraic Representations \\of Network Behavior}\label{sec:representations}
...
\end{tabbing}
The total behavior matrix is the concatenation of these matrices: $F = (S,D,B,E,Z,V) \in \R^{f \times \Sigma}$ where $\Sigma=2(n+d_t)+d_z+d_v$. Many common traffic models can be expressed precisely and succinctly in terms of transformations or conditions on this behavior matrix and its constituents.
...
The number of flows between each pair of nodes in the network is immediately given by the matrix product $SD^t\in\R^{n \times n}$, where $*^t$ denotes the transpose. That is $(SD^t)_{ij}$ is the number of flows from node $i$ to node $j$. The total number of source-flows for each node is given by the vector $S^t\ones{f}\in\R^n$, while the total number of destination-flows is given by $D^t\ones{f}\in\R^n$ ($\ones{f}$ is the $f$-dimensional column vector consisting of all ones).

\subsection{Average Flow Behaviors}

The average packet sizes and inter-packet intervals for flows are computed by right multiplication by the vector of values corresponding to the column indices. For example, the packet sizes corresponding to the columns of the size matrix are simply the indices themselves: $\vec{\sigma}=(1,2,3,\dots,d_z)$. Since inter-packet intervals were quantized according to \Equation{interval-quantization}, we use the median value of the $k$th interval to represent it:
\begin{align}
\vec{\tau}(k) = v_{\min}+(v_{\max}-v_{\min})\fracp{k(k+1)+\frac{1}{2}}{d_v^2}.
\end{align}
With these index-value vectors, the average packet sizes and inter-packet intervals for each flow can be computed:
\begin{align}
\label{eqn:average-size}%
\text{average sizes:}&~~~~~Z\vec{\sigma} \in \R^f,\\
\label{eqn:average-interval}%
\text{average intervals:}&~~~~~V\vec{\tau} \in \R^f.
\end{align}

%Computing the aggregate behavior is exceedingly simple. It is generally accomplished through 
...
It is common in generating traffic behaviors for experiments to treat packets, nodes, and flows uniformly to varying degrees, largely because of the high complexity of doing otherwise and the lack of compelling alternatives. Here we examine how various notions of uniformity are represented in the matrix model. In general, \textit{intra-flow uniformity} is effected by right multiplication, which mixes the elements in each row, whereas \textit{inter-flow uniformity} is effected by left multiplication, which mixes elements across rows. The standard constant bit-rate (\caps{CBR}) flow model, for example, in its strictest sense, is an intra-flow model, which stipulates that all the packets in each flow behave identically, having identical sizes and intervals. Each flow's bit-rate is therefore constant, and hence the name. The strict \caps{CBR} model can be effected by using the expressions for per-flow average packet sizes and inter-packet intervals given in Equations~\ref{eqn:average-size}~and~\ref{eqn:average-interval}. This essentially reduces the size and time representation to the 
