...
These transformations, however, are extremely destructive: the matrices are degenerate with very low rank,\ranknote so multiplication destroys almost all information in the original traffic matrix.

\input{trace_vs_uniform}

We argue that matrix factorization, which is nondestructive, is a far superior way to reduce the dimensionality of network traffic behaviors.
...
Although the basic behaviors are found algorithmically, they correspond to intuitive network activities such as file transfer, typing via \caps{SSH}, web surfing, and ping traffic.
