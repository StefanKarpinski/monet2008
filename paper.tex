...
More than twenty years of research in analysis and modeling of network traffic have yielded tremendous advances in our understanding of the complex behaviors that emerge from the interaction of millions of humans and computers on the Internet and in packet-switched networks.
...
\item \textbf{Subdivided Independence:} that traffic properties behave independently, but only within certain groupings. The assumption, for example, that all flows emanating from a node share the same distribution of packet sizes is of this type.
\end{enumerate}
...
Simple examples illustrate this: downloading a large file vs. typing in an \caps{SSH} session; BitTorrent users vs. occasional email checkers.
...
The second reason why uniformity and independence assumptions are so commonly used is that there are no generally accepted alternatives for representing or analyzing traffic.
...
Prior research has indicated that, given accurate per-flow distributions, this is a fairly realistic model of network behavior~\cite{Karpinski07:realism}.\realismnote
Such empirical distributions, however, are complex entities unto themselves---and worse still, each flow has its own distribution.
Modeling the entire network's behavior thus entails producing a distribution of distributions.
...
Uniform and independent modeling reduce the dimensionality of network behavior drastically, making analysis and generation tractable problems.
There are two immediate questions:
\begin{enumerate}
\item Can we represent the original ``unreduced'' network behavior in such a way that analysis and generation are still tractable problems?
...
\begin{quote}
The sum of the representations of two flows must be another vector that represents the aggregate behavior of the two flows.
\end{quote}
