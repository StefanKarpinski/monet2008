...
\newcommand{\thetitle}{An Algebraic Representation of Wireless Workload}
...
The simplifying assumptions made by most traffic models can be classified into three categories. We use the terms \textit{uniformity}, \textit{marginality}, and \textit{regularity} do describe these classes of assumptions. They are all assumptions about the ``sameness'' of aspects of network behaviors; they differ, however, in whether the make stipulations about uniformity within each flow, across all flows, or within groups of flows.
...
Since uniform behaviors entail homogeneity of properties with respect to each single flow individually, a trace matrix can be transformed to the closest matrix satisfying the model's assumptions via right matrix multiplication. Right multiplication mixes the elements within each row, and if the mixing weights are all equal, the results is homogenized behavior. For example, to make the roles of the nodes with respect to each flow statistically identical, we transform $S$ and $D$:
\begin{align*}
...
Marginality is an orthogonal concept to uniformity: it implies that the distribution of some property is identical \textit{across} all flows. For example, if we determine a distribution of flow sizes across the entire network and assume that this distribution applies \textit{independent of other flow properties}, this is a marginal model for flow size. The traffic models proposed by {\FHC}~\cite{Hernandez06:wlan-traffic} are all marginal: they propose certain network-wide parametric distributions for start times, flow sizes and the number of flows per node. These models do not account for non-independence between these and other flow properties. We will discuss this limitation and its implications when we analyze our experimental results.

\subsubsection{Regularized Behavior}

The final form of sameness we consider, is regularization. This is a restricted form of marginality: we may assume that marginal distributions apply to all the flows in certain groupings. For example, we can provide a distribution of flow sizes on a per source-node basis; that is, each source node can have it's \textit{own} distribution, and the sizes of its flows are drawn from that distribution, independent of their other properties.
...
full uniform & \textit{all uniform transformations} & applies all the uniformity assumptions of the above three models \\\hline
\hline
time marginal & $A'=(\ones{f \times f}/f)A$ & start time independently chosen from overall distribution of start times \\\hline
...
full marginal & \textit{all marginal transformations} & applies all the marginality assumptions of the above four models \\\hline
\hline
...
\item \textbf{Network:} \caps{AODV} control overhead (\caps{RREQ/RREP/RERR}), packet drops in \caps{IP} routing queues.
...
{\includegraphics[width=4.6in]{plots/Application/delivery_ratio.pdf}}
