...
Fundamentally, uniform, marginal and conditional models are applied so that we can represent network traffic behavior using a few empirical or analytical statistical distributions.
Reduction in representation size and complexity makes analysis and generation tractable problems.
From this perspective, two questions immediately present themselves:
\begin{enumerate}
\item Can we represent ``unreduced'' network behavior such that analysis and generation are tractable?
\item Are there better ways to reduce the representation of network behavior?
\end{enumerate}
...
To address the question of whether there are better ways to reduce the representation of network behaviors, we first make a simple observation.
...
We argue that matrix factorization, which is nondestructive, is far superior to multiplication for reducing the representation of network traffic behaviors.
To demonstrate this approach, we use non-negative matrix factorization (\caps{NMF}) to find a low-dimensional approximation of the packet size and inter-packet interval matrices of a real traffic trace.
...
Although basic behaviors are computed without examining port numbers, they nevertheless correspond to the intuitive behaviors expected for protocols.
Basic behaviors are found corresponding to typical network activities, including file transfer, typing via \caps{SSH}, web surfing, ping traffic, peer-to-peer and others.
...
\caps{UDP} flows are modeled as being constant bit-rate; %(\latin{i.e.} having uniform packet behavior),
...
Inter-connection times %(\latin{i.e.} per node-pair inter-flow intervals)
...
Previous research has shown that uniform packet behavior models used in Harpoon drastically distort vital performance metrics at all levels of the protocol stack~\cite{Karpinski07:cbr-failure}. % in wireless network simulations.
...
Packet behavior can be modeled using standard statistical distributions (``constant, uniform, normal, cauchy, pareto, exponential, etc.''). % for packet size and inter-packet intervals.
...
The focus of this project is primarily on the engineering challenge of generating very large volumes of synthetic traffic and injecting it into real networks in a distributed fashion.
