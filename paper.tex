...
\newcommand{\class}[1]{\textsc{\smaller{#1}}}
\newcommand{\model}[1]{\textsf{\smaller{#1}}}
...
One of the major challenges of modeling full-network traffic is the multilevel interdependence of behaviors.
% The packet is the fundamental unit of traffic in any packet-switched network, including \caps{IP} networks.
% Packets are aggregated at a variety of logical levels: by protocol type, by source and destination nodes, by flows sequences between communicating applications.
% The behaviors of packets are specific to each aggregated group: the distribution of packet sizes is specific to each node as both source and destination, and to each flow as well.
% Because of the lack of adequate techniques for modeling this complex multilevel interdependence,
Three types of simplifying assumptions are commonly applied to both analysis and generation of traffic:
\begin{enumerate}
\item \textbf{Uniformity:} that certain aspects of traffic behavior are uniformly distributed. For example, that  all nodes in a network have equal probability of being the source or destination of packets or flows.%
\footnote{We use the common definition of a \textit{flow}: a sequence of packets sharing the same \caps{IP} protocol type, source and destination nodes, and \caps{TCP/UDP} port numbers.}
\item \textbf{Independence:} that traffic properties share a single marginal%
\footnote{The term ``marginal'' refers to the margins of actuarial tables formerly used for statistical computations. The rows and columns of the table represent possible values of two properties. Each entry in a table contains a count of the number of events falling into the joint category for that row and column. The margins contain sums of the rows or columns, thus giving the distribution of properties along each dimension \emph{independent} of each other.}
distribution across the entire network, and individual values are independently drawn from this marginal distribution. For example, assuming that a single distribution describes an entire network, as opposed to modeling each flow with its own distribution of packet sizes.
\item \textbf{Subdivided Independence:} that traffic properties behave independently, but only within certain groupings. The assumption, for example, that all flows emanating from a node share the same distribution of packet sizes.
\end{enumerate}
...
They clearly are not: flows have drastically different packet size distributions; different nodes initiate and receive drastically different numbers of packets and flows. Consider downloading a large file vs. typing in an \caps{SSH} session, and BitTorrent users vs. occasional email checkers.

If no one actually believes that assumptions of uniformity and independence accurately describe network behavior, then why are these assumptions so common? The first reason is simply that these assumptions drastically simplify both traffic analysis and workload generation. If we ignore the interdependencies that make analysis difficult, the difficulties simply evaporate. 
% The answer is twofold:
% \begin{inparaenum}[1)]
% \item they drastically simplify both analysis and workload generation;
% \item there is no generally accepted alternative for traffic analysis.
% \end{inparaenum}
% The first point is clear: if we ignore the interdependencies that make analysis difficult, the difficulties evaporate.
This immediately begs the question of whether these assumptions are benign.
% The question remains making these assumptions is benign.
Does ignoring the non-uniformity and non-independence of network behavior distort the properties and metrics we are trying to analyze and replicate?
The first contribution of this paper is to answer this question in the negative: we cannot stick our heads in the sand and pretend that network properties are uniform or independent without severely distorting crucial network performance metrics.

The second reason why uniformity and independence assumptions are so commonly used is that there is no generally accepted alternative for analyzing traffic.
%There is no proven, generally accepted approach to representing or analyzing the multilayered statistical interdependence of traffic patterns other than simply assuming that these interdependencies do not exist.
%(Worse still, such assumptions are often made implicitly and their limitation never stated outright.)
The difficulty lies in the high dimensional and stochastic nature of network behaviors.
Suppose that we assume that packet sizes and inter-packet intervals are drawn from some distribution for each flow.
Prior research has indicated that, given accurate per-flow distributions, this is a realistic generative model of network behavior~\cite{Karpinski07:realism}.%
\footnote{It is realistic in the sense that traffic generated using this model accurately reproduces performance metrics of real traffic.}
However, such empirical distributions are complex entities unto themselves---and worse still, each flow has its own distribution.
Modeling the entire network's behavior thus entails producing a distribution of distributions---with only one sample value from each distribution.
% How can we know if two flows' observed behaviors differ because they were fundamentally different or because the stochastic processes driving them happened to produce different-looking results?

Uniform and independent modeling collapses network behavior (in a way that we will make precise later) to drastically reduce the dimensionality of the representation.
This observation yields two immediate questions:
\begin{enumerate}
\item How can we represent the original ``uncollapsed'' network behavior?
\item Is there a better way than assuming independence or uniformity to reduce dimensionality?
\end{enumerate}
It is already common practice in network modeling to think of and represent flow behaviors as vectors: the \caps{IP} protocol number, source and destination \caps{IP} addresses, and \caps{UDP/TCP} source and port numbers that define a packet flow are commonly referred to as a 5-tuple---i.e., a five-dimensional vector. The empirical distributions of packet sizes and inter-packet intervals that describe each flow's packet behavior are also naturally represented as histogram vectors: each dimension represents a bin and corresponding coordinate represents the number of packets falling into that bin. We argue that the natural generalization of this vectorial representation of flow behaviors to a collection of flows is to represent the entire collection as a matrix.
...
% The question remains whether assumptions of uniformity and independence are benign or whether they severely distort the properties of network behavior we are trying to analyze or replicate experimentally.
% %Is the analysis and generation of network traffic significantly distorted by such assumptions?
% This is the first fundamental question that this paper addresses. The that uniformity assumptions has already been made in earlier papers~\cite{Karpinski07:cbr-failure}.
% Therefore, we focus here on addressing assumptions of independence or subdivided independence. We find that assuming independent behaviors across entire networks is also extremely distorting of important measures of network performance.
% %This research has shown that assumptions of constant bit-rate (\caps{CBR}) flow behavior, as well as assumptions of uniformity at the flow and node levels of behavior drastically distort important performance metrics when used to generate wireless workload traffic for experiments.
% 
% While these assumptions certainly simplify both analysis and workload generation, they are quite certainly not true: flows do not all share a single packet size distribution; every node is not equally likely to be the source or destination of each packet or flow.
% 
...
%This does not solve the ``cookbook'' problem, however, because the \caps{GMM} must still be based on real traffic in order to reproduce its behavioral characteristics. It does however, reduce the problem of general traffic modeling to a more constrained problem of matrix modeling.

%Our experiments with various common assumptions of uniformity and regularity in network behavior yield mixed conclusions: most straightforward assumptions of homogenous behavior distort performance results significantly, but there are notable exceptions. The clearest exception is the time domain: traffic models appear to be relatively insensitive to local perturbation of the start time of flows---so long as the overall distribution remains approximately correct. Our conclusion is that traffic modeling needs to discard its assumptions of uniformity, regularity and independence. Instead we need to move ahead trying to understand the complex dependencies between the many aspects of traffic behaviors. By reducing traffic modeling to the well-understood domain of numerical linear analysis, the General Matrix Model brings us significantly closer to the ultimate goal of having a ``cookbook'' model for generating arbitrary, realistic experimental workload patterns.
...
\begin{align}
  \mean{z}&=\fracx{\inner{\vec{z}}{\vec{\sigma}}}{\inner{\vec{z}}{\ones{d_z}}},\\
  \mean{v}&=\fracx{\inner{\vec{v}}{\vec{\tau}}}{\inner{\vec{v}}{\ones{d_v}}},
\end{align}
