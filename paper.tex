...
\newfootnote{\onesnote}{%
Through out this paper, $\ones{}$ denotes a matrix of all ones, but the dimensions of the matrix must be inferred by context.
Where necessary, we disambiguate the dimensions by explicitly specifying the dimensions of the final matrix product.
}
...
\end{align}
The packet count, total byte and total duration vectors are computed as\onesnote
\begin{align}
...
\end{align}
Here $\ones{}$ is a row vector of all ones.
...
For example, we may compute the packet transmission graph:
\begin{align}
