...
\section{Derivative Representations of Network Behavior}

Now that we have defined our representation of network behavior, we derive useful expressions for commonly used network properties in terms of these basic representations.

\subsection{Communication Topology}

The number of flows between each pair of nodes in the network is immediately given by the matrix product $SD^t\in\R^{n \times n}$. The entry at row $i$ and column $j$ in this matrix gives the number of flows from 
