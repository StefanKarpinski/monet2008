...
The {packets-bytes-duration} model representation is clearly quite simplistic, not to mention incomplete. It captures nothing about variability in packet sizes, nor about the ``burstiness'' of packet transmissions; it leaves start time unspecified; and it says nothing about the source or destination of the flow. At the opposite extreme, it is possible to embed the exact start time of each flow, and the exact size and transmission times of its packets in a vector space. The difficulty comes in doing so in a manner that makes the summation of vectors correspond naturally and meaningfully to aggregate behavior of flows. We take an intermediate approach, suggested by the results of~\cite{Karpinski07:realism}: if the end-points, start time and end time of each flow is known, it suffices to know the empirical distribution of packet sizes and inter-packet intervals in order to reproduce network performance characteristics accurately. Accordingly, we represent a flow's behavior by these precise characteristics. We encode a single flow's behavior with six component vectors:
\begin{itemize}
