...
\begin{center}\small
\input{models.tex}
\caption{The three orthogonal levels of traffic behavior, and the traffic models used for each level.}\label{tab:traffic-models}
\end{center}
\vspace{-1.75em}
\end{table*}
...
\end{enumerate}
For each level of traffic behavior, we compare several different behavior models. The different levels of models and variants at each level are listed and described in Table~\ref{tab:traffic-models} and illustrated with specific examples in Figure~\ref{fig:traffic-models}.
...
We have selected nine performance metrics at the application, network, and link layers of the protocol stack:
\begin{enumerate}
\setlength{\itemsep}{0em}
\item \textbf{Application:} average end-to-end delay, average jitter, total received throughput.
\item \textbf{Network:} \caps{AODV} control overhead (\caps{RREQ/RREP/RERR}), \caps{RREQ}s initiated per node, routing queue drop rate.
\item \textbf{Link (\caps{MAC}):} control overhead (\caps{RTS/CTS/ACK}), packet retransmission rate, retransmission failure rate.
\end{enumerate}
