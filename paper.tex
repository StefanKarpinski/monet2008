...
\newcommand{\thetitle}{On the Inadequacy of Uniform Modeling for Wireless LAN Workload Generation}
...
Table~\ref{tab:properties} enumerates all the properties exposed by traffic models used in this work. Other traffic models may necessitate exposing additional properties, but this list suffices to implement the models used in this paper. Concrete classes implement the behavior of specific traffic models at each level. A complete synthetic workload is instantiated as follows:
\begin{enumerate}
\item create a \class{Network} object of the appropriate class;
...
For each of the \class{Node}, \class{Flow} and \class{Packet} abstract classes, we create define four concrete subclasses: \class{Trace}, \class{Nonparametric}, \class{Parametric} and \class{Uniform}. The \class{Trace} class at each level takes its properties directly from the behavior of the original traffic. The \class{Nonparametric} and \class{Parametric} classes use nonparametric models of marginal behavior at each level to 
