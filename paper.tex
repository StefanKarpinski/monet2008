...
%For example, if there are 3 nodes in a workload, and the source vector for a flow is $\vec{s}=(3,7,1)$, we choose a source node $k$ with $\Pr(k=1)=\frac{3}{11}$, $\Pr(k=2)=\frac{7}{11}$, and $\Pr(k=3)=\frac{1}{11}$. For inter-packet intervals, a bin is chosen in the same manner, and a random value from the range represented by the bin is generated.
