...
% In this section, we the concept of linear representation of network traffic. Before we introduce this concept, we discuss the characteristic features of network traffic from the traffic modeling perspective.
...
its start time;
and the specific sizes of and intervals between its constituent packets.
...
\end{align}
Since flows contain only a finite number of packets, there exists a number of packets, $\pkts \in \N$, for each flow. Thus, we require that the sequences $\sizes$ and $\ivals$ satisfy the following requirements:
\begin{align}
...
\end{align}
% We define the function $\pkts:\flow \to \N$ as
% \begin{align}
%   \pkts(\flow) \defeq \min\set{i \in \N : \ival_i = \infty}.
% \end{align}
% Intuitively, by Equations~\ref{eqn:finite-intervals}~and~\ref{eqn:infinite-intervals}, this gives the number of packets in the flow.
...
A \defterm{linear representation} of network traffic is a pair, $\tuple{V,\psi}$, where $V$ is a vector space and $\psi$ is a function, $\psi: \Traffic \to V$, that maps each traffic sample to the sum of the representations its constituent flows:
\begin{align}\eqn{linearity}
\forall~T \in \Traffic\!:\quad
  \psi(T) &= \sum_{\mathclap{\flow \in T}}{\psi(\flow)}.
\end{align}
% When $T=\emptyset$, the right-hand side is a sum of zero items, defined to be $\vec{0}$, the identity element of $V$.
\end{definition}
...
\Equation{linearity} defines the extension to all of $\Traffic$.
Representing flows by arbitrary vectors, however, is not particularly interesting or useful.
Therefore, we focus on the meaningful representation of collections of flows, that can be factored or explained in terms of their constituent individual flows.
In the next section, we clarify the concept and application of linear representation through examples.
% we present and discuss specific examples of linear representations in order to clarify the concept and its application.

\subsection{Examples of Linear Representations}
\sec{linear-representation-examples}

\newfootnote{\pbdnote}{There are many schemes to correct the discrepancy in bytes when $\pkts$ does not divide $\mathsym{bytes}$ evenly. The simplest is to ignore them; we leave leave more complex schemes to the reader.}

Our first example is the packets-bytes-duration (\caps{PBD}) representation.
The representation is $\pbd: \Traffic \to \R^3$, mapping
$\flow \mapsto \tuple{p,b,d}$, where
\begin{align}
p &= \pkts, &
...
With care, the sum of representations of flows can still naturally describe the aggregate behavior of  flows.
We will define the representation $\Phi: \Traffic \to \FlowVec$, where
\begin{align}
...
Here $L^2[\R]$ is the usual vector space of real functions~\cite{Rudin87}: $\R^\R=\set{f: \R \to \R}$ equipped with the norm,
\begin{align}
\norm{f}_2 = \Big[ \int_{-\infty}^{\infty} \abs{f}^2 dx \Big]^{\frac{1}{2}}.
\end{align}
We define $\Phi(\flow) = \tuple{\e_\type,\e_\src,\e_\dst,f,g}$, where $\e_k$ is the $k$th standard basis vector, and $f,g \in L^2[\R]$ are
\begin{align}
f(t) &= \sum_{k=1}^{\pkts}
  \begin{cases}
    1 & \text{if $t \ge \time_k$} \\
...
g(t) &= \sum_{k=1}^{\pkts}
  ...
  \end{cases}
\end{align}
Thus, $f$ and $g$ are both integer-valued, monotonically increasing step functions, giving the cumulative number of packets sent and total bytes sent, respectively, at time $t$.
From the representation via $\Phi$ of a single flow, we can readily recover its type, source and destination, and the transmission times and sizes of packets.
...
Vector addition in $L^2[\R]$ dictates that these functions give the total number of cumulative packets and bytes, respectively, over all flows in $T$.
Thus, summation of vectors under the representation $\Phi$ yields a natural and useful description of the aggregate behavior of many flows.
...
Representations that preserve even more information about flows can be constructed.
We turn now in a different direction, however, and introduce a linear representation that strikes a practical balance in fidelity between $\pbd$ and $\Phi$.
