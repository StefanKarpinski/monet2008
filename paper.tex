...
The {packets-bytes-duration} model representation of flow behavior is clearly quite simplistic. It captures nothing about variability in packet sizes, nor about the ``burstiness'' of packet transmissions. At the opposite extreme, it is possible to embed the exact start time of each flow, and the exact size and transmission times of its packets in a vector space. The difficulty comes in doing so in a manner that makes the summation of vectors correspond naturally and meaningfully to aggregate behavior of flows. Instead, we take an intermediate approach, suggested by the main result of~\cite{Karpinski07:realism}: if the start time and end time of each flow is known, it suffices to know the empirical distribution of packet sizes and inter-packet intervals in order to reproduce network performance characteristics accurately. Accordingly, we represent a flow's behavior by these precise characteristics.

Explicitly, we encode a flow's behavior with four vectors: $\vec{z} \in \R^{d_z}$ is a histogram of packet sizes; $\vec{v} \in \R^{d_v}$ is a histogram of inter-packet intervals; $\vec{\alpha}, \vec{\epsilon} \in \R^{d_t}$ are vectors indicating when the flow begins and ends. To expand on this, $z_i$ (i.e. the $i$th element of $\vec{z}$) is the number of packets in the flow such with $i$ bytes and $d_z$ is the largest possible packet size, which in practice is the maximum transfer unit of the wireless medium---typically 1500 bytes. The histogram of inter-packet intervals is similar, except that domain of possible intervals is a continuum, so we much map the range of possible intervals onto the discrete index values $\left\{1,\dots,d_v\right\}$. In our experiments, we use the following logarithmic quantization function:
\begin{align}
q(v) &= \left\lfloor(d_v-1)\frac{\ln v - \ln v_{\min}}{\ln v_{\max}-\ln v_{\min}}\right\rfloor+1.
\end{align}
with $v_{\min}=10^{-6}$, which is the minimal interval our trace data, and $v_{\max}=600$, which is the total duration of each simulated situation, and hence an upper bound on the inter-packet intervals. We use $d_v=1500$ to match the dimensionally of the packet size representation. 

The flow start and end time indicator vectors, $\vec{\alpha}$ and $\vec{\epsilon}$, also require some explanation. Start and end times are simple scalar values. Why not represent them as such? The answer lies in the requirement that behaviors must add meaningfully. If we represent the start time as a scalar, then adding the behavior of two flows produces a (fairly) meaningless value. It's certainly not a start time itself---it may even exceed the start time of any actual flow. Instead, we again quantize the continuum of time: we map the range of possible start times onto the indices $\left\{1,\dots,d_t\right\}$, where $d_t$ is some chosen dimensionality. If the start time is mapped to index, $i$, then $\vec{\alpha}=e_i$, the vector with 1 at index $i$ and zeros elsewhere. The benefit of this representation is that we can now appropriately represent aggregate behavior through vector addition: the sum of start vectors over any collection of flows represents is a histogram of start times for those flows.

Finally, we collect all the pieces of flow behavior into matrix representations of behavior across an entire traffic trace. If there are $f$ flows in the trace, we define the size matrix $Z\in\R^{f \times d_z}$ to be the matrix whose row vectors are the size vectors $\vec{z}_i$ for $i=1,\dots,f$. In precisely the same manner, we define the interval matrix, $V\in\R^{f \times d_v}$, and the start time matrix, $A\in\R^{f \times d_t}$, and the end time matrix, $E\in\R^{f \times d_t}$.

\subsection{Flow Sources \& Destinations}
